...
class VanillaTemporalModule(nn.Module):
    def __init__(
        ...
        masks_dir = Path(__file__).resolve().parents[2] / "masks"
        self.mask_64 = self.create_mask(masks_dir / "mask_64.png", b, c_64, f)
        self.mask_32 = self.create_mask(masks_dir / "mask_32.png", b, c_32, f)
        self.mask_16 = self.create_mask(masks_dir / "mask_16.png", b, c_16, f)
        self.mask_8 = self.create_mask(masks_dir / "mask_8.png", b, c_8, f)
    ...
    def create_mask(self, mask_path, b, c_new, f):
        raw_mask = cv2.imread(str(mask_path))
        if raw_mask is None:
            raise FileNotFoundError(f"HighSync motion mask could not be read: {mask_path}")
        mask = torch.FloatTensor(1 - (raw_mask / 255.))
        (h, w, c) = mask.shape
        ...
        return mask_new.to(device, dtype=torch.float16)
    ...
    def forward(
        ...
        hidden_states = input_tensor * mask
        hidden_states = self.temporal_transformer(
            hidden_states, encoder_hidden_states, attention_mask
        )

        output = hidden_states*mask + input_tensor*(1-mask)
        # output = input_tensor
