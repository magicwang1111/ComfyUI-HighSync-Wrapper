...
class StaticPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        gt_image_tensor = self.ref_image_processor.preprocess(
            gt_image, height=height, width=width)  # (bs, c, width, height)
        gt_image_tensor = gt_image_tensor.to(dtype=self.vae.dtype, device=self.vae.device) # f*c*h*w
        #do vae for gt image
        gt_encode = self.vae.encode(gt_image_tensor).latent_dist.mean
        gt_encode = gt_encode.unsqueeze(2) # b*c*f*h*w
        gt_encode = gt_encode * 0.18215

        # audio_tensor = torch.zeros((gt_encode.shape[0],50,384))
        audio_tensor = torch.tensor(audio_tensor).to(dtype=self.denoising_unet.dtype, device=self.denoising_unet.device).unsqueeze(0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = latents*mask + gt_encode*(1-mask)
