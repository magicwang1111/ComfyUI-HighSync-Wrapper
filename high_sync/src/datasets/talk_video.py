...
class TalkingVideoDatasetVal(Dataset):
    """
    ...
    def create_mask(self, mask_path):
        ...
        # mask = mask.repeat(1,self.n_sample_frames,1,1)
    ...
    def __getitem__(self, index):
        ...
        video_path = video_meta["video_path"] #"/workspace/preprocess/videos/ga3GzQWqWWg_C0.mp4"
        same = False #random.choice((True, False))
        if same:
            audio_emb_path = video_meta["audio_emb"]
            audio_path = audio_emb_path.replace("audio_emb_whisper", "audios")[:-2] + "wav"
        else:
            if (index+1) > (len(self.vid_meta)-1):
                new_index = index - 1
            else:
                new_index =  index + 1
            video_meta_next = self.vid_meta[new_index]
            audio_emb_path = video_meta_next["audio_emb"]
            audio_path = audio_emb_path.replace("audio_emb_whisper", "audios")[:-2] + "wav"
        ...
        frame_list = [
            Image.fromarray(video).convert("RGB") for video in video_frames[:5*25]] #.asnumpy()

        audio_emb = torch.tensor(torch.load(audio_emb_path))
...
class TalkingVideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        video_length = min(len(video_frames), audio_emb.shape[0])
