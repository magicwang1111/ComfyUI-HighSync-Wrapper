...
from .src.pipelines.face_animate_static import StaticPipeline
from .src.datasets.mask_image import FaceMaskDataset, FaceMaskDatasetVal
...
class Net(nn.Module):
    ...
    def forward(
        ...
        # audio_emb = torch.zeros((noisy_latents.shape[0],50,384)).to(
        #     device=self.reference_unet.device, dtype=self.reference_unet.dtype)
        audio_emb = audio_emb.to(device=self.reference_unet.device, dtype=self.reference_unet.dtype)
...
def log_validation(
    ...
    sample_idx = [np.random.randint(0, dataset_len-1) for _ in range(2)]
    for idx in sample_idx:
        sample = val_dataset[idx]

        ref_image_pil = sample['ref_img']
        # gt_image_pil = Image.fromarray(sample['tar_gt']).convert("RGB")
        # ref_image = sample['ref_img'] # h*w*c
        # gt_images_no_trans = sample['tar_gt_no_trans'] # f*c*h*w
        gt_image = sample['img'] # f*c*h*w
        mask = sample['mask'].unsqueeze(0).to(device="cuda", dtype=weight_dtype) # b*c*f*h*w
        # mask_512 = sample['mask_512'].unsqueeze(0) # b*c*f*h*w
        ...
        # gt_images_no_trans = gt_images_no_trans.permute(1,0,2,3).unsqueeze(0) # b*c*f*h*w
        # image = image * mask_512 + gt_images_no_trans * (1-mask_512)
        image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (c, h, w)
        res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
        # Save ref_image, src_image and the generated_image
        w, h = res_image_pil.size
        canvas = Image.new("RGB", (w * 3, h), "white")
        ref_image_pil = sample['ref_img'].convert("RGB").resize((w, h))
        gt_image_pil = sample['img'].convert("RGB").resize((w, h))

        canvas.paste(ref_image_pil, (0, 0))
        canvas.paste(res_image_pil, (w, 0))
        canvas.paste(gt_image_pil, (w * 2, 0))

        out_file = os.path.join(save_dir, f"{global_step:06d}-{idx}.jpg")
...
def train_stage1_process(cfg: argparse.Namespace) -> None:
    ...
    denoising_unet.load_state_dict(
        torch.load(
            os.path.join(cfg.echo_path, "denoising_unet-220000.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    reference_unet.load_state_dict(
        torch.load(
            os.path.join(cfg.echo_path, "reference_unet-220000.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    ...
    val_dataset = FaceMaskDatasetVal(
        img_size=(cfg.data.train_width, cfg.data.train_height),
        mask_path=cfg.data.mask_path,
    ...
    for _ in range(first_epoch, num_train_epochs):
        ...
        for _, batch in enumerate(train_dataloader):
            with accelerator.accumulate(net):
                ...
                uncond_fwd = np.random.random() < cfg.uncond_ratio

                with torch.no_grad():
                    ref_img = batch["ref_img"].to(
                ...
                if cfg.snr_gamma == 0:
                    ...
                    loss = F.mse_loss(
                        (model_pred*mask).float(), (target*mask).float(), reduction="none"
                    )
                    loss = (
